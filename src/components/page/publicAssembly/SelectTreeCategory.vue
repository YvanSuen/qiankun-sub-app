...
export default {
  ...
  created() {
    ...
    if (this.value) {
      ...
      this.threeModel = this.queryTreeID(this.data, this.value);
    }
    // 获取输入框宽度同步至树状菜单宽度
    this.$nextTick(() => {
  ...
  methods: {
    ...
    onClickNode(node) {
      ...
      this.threeModel = node[this.props.valueThree];
      this.onCloseTree();
    },
    // 隐藏树状菜单
    onCloseTree() {
    ...
    queryTreeID(tree, id) {
      ...
      while (stark.length) {
        ...
        if (temp[this.props.value] === id) {
          return temp[this.props.valueID];
        }
      }
      return '';
    },
    async getdateDept(){
      ...
      await axios.post(EamWebApiUrl+"/EamCategory/QueryCategoryTreeList",  params).then((res) => {
