    ...
    <div>
        <div class="crumbs">
            <el-breadcrumb separator="/">
                <el-breadcrumb-item>
                    <i class="el-icon-thirdkucun"></i> 车间人员权限
                </el-breadcrumb-item>
            </el-breadcrumb>
        </div>
        <div class="container">
    ...
    methods: {
        ...
        handleCheckChange(data, checked, indeterminate) {
         ...
         handleNodeClick(data) {
            this.$set(this.query, 'pageIndex', 1);
             let strWhere = '1=1';
             strWhere = strWhere + " and T.PersonCode in(SELECT PersonCode FROM[dbo].[v_LibFactoryModelPerson] WHERE FactoryModelID = '" + data.id + "')";
        ...
        Save() {
            ...
            var worklist = this.$refs.tree.getCheckedNodes();
            ...
            for (var i = 0; i < worklist.length; i++) {
                if(worklist[i].LayType=="AREA"){
                let jsonData = {
                    PersonID: this.selectedRow.PersonID,
                    PersonCode: this.selectedRow.PersonCode,
                    FactoryModelID: worklist[i].id,
                    FactoryModelCode: worklist[i].code,
                    CreateUser: localStorage.getItem('ms_username'),
                    CreateDate: new Date()
                };
                worklistlog=worklistlog+worklist[i].name+"|";
            ...
            let json = JSON.stringify({
                ...
                MenuName: '人员车间授权',
                ObjectName: 'SetSysFactoryModelPerson',
                CreateUser: localStorage.getItem('ms_username'),
                Operation: '〔授权〕人员：' + this.selectedRow.PersonCode + '-' + this.selectedRow.PersonName + ' - 车间权限：'+worklistlog
            });
            params.append('Logjson', json);
             showLoading();
            axios.post(BaseWebApiUrl + '/SysFactoryModelPerson/SetSysFactoryModelPerson', params).then((res) => {
                hideLoading();
                if (res.data.returnCode == '1') {
        ...
        onRowClick(row, event, column, cell) {
            ...
            axios.post(BaseWebApiUrl + '/SysFactoryModelPerson/GetSetPersonList', params).then((res) => {
                let newArr = [];
                if (res.data.returnCode == '1') {
        ...
        getTreeData() {
            ...
            axios.post(BaseWebApiUrl + '/SysFactory/QuerySysFactoryPersonTreeList', params).then((res) => {
                if (res.data.returnCode == '1') {
